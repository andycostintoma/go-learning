...
The third 01_goroutine reads all the numbers from the channel and prints them out.
...
func ProcessData() {
	...
	// launch this helper 01_goroutine to close the channel when the two writing goroutines are done
	go func() {
		wg.Wait()
		close(ch)
	}()
	// the second waitgroup signals when the reading 01_goroutine is done
